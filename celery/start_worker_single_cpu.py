...
if __name__ == "__main__":
    ...
    logname = '{host}_single_cpu_{dt}.log'.format(host=host, dt=dt)
    logfile = os.path.join(config.LOG_PATH, logname)

    cstr = 'celery -A tasks worker -n {host}_single_cpu -c {ncores} -Q {host}_single_cpu -E -l INFO -f {logfile}'.format(host=host, ncores=ncores, logfile=logfile)
    print(cstr)
    os.system(cstr)
