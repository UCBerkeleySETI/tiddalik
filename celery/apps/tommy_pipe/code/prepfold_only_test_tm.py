...
for cand in root.findall('candidates/candidate'):
    ...
    if snr >= 10:
        cmd = 'prepfold -noxwin  -dm {0} -nsub 88 -npart 10 -n 2 -npfact 3 -ndmfact 24 -p {1} -mask ../{2} ../../{3}'.format(dm, period, mask_file_name, fil_file)
        print (cmd)
...
#        print fold_out
        ...
        if os.path.exists(mask_file_name):
            cmd = 'prepfold -noxwin  -dm {0} -nsub {1} -npart {2} {3} -n {4} -npfact {5} -ndmfact {6} -p {7} -mask ../{8} ../../{9}'.format(dm, 88, npart, otheropts, N, Mp, Mdm, period, mask_file_name , fil_file)
            print period, dm, snr
            print (cmd)
    #         subprocess.call(cmd, shell=True)
            job_prepfold.append(cmd)
            ...
            cmd = 'prepfold -noxwin -dm {0} -nsub {1} -npart {2} {3} -n {4} -npfact {5} -ndmfact {6} -p {7} ../../{8}'.format(dm, 88, npart, otheropts, N, Mp, Mdm, period, fil_file)
            print period, dm, snr
            print (cmd)
     #        subprocess.call(cmd, shell=True)
            job_prepfold.append(cmd)
