...
'''    
####################################################################################################
# This parallelises the prepfold tasks
...
def excute_cmd (cmd):
    ...
    def process_prepfold (self):
        ...
        for candidate in candidate_list:
            cmd = 'prepfold {0} {1}'.format(working_dir, os.path.join(working_dir, obs_calib))
            job_prepfold.append(cmd)

        pool = Pool(self.ncpus)
        pool.map(excute_cmd, job_zapCal)
        pool.close()
        pool.join()


######################################################################################################
# Thhese commands will be rolled into the above section
...
pdf_file_name = 'pdf' +  cand_dir[:-1] + '.pdf'
