...
#####################################################################
# rename fil file
fil_file.split("_0001.")
new_fil_name = str(fil_file.split("_0001.")[0]) + '_' + str(fil_file.split("_0001.")[1])
new_fil_name = str(new_fil_name)
os.environ['new_fil_name'] = new_fil_name
os.environ['fil_file'] = fil_file
print ("Renaming filterbank file {0} to {1}".format(fil_file, new_fil_name)) 

cmd = 'mv ./{0} ./{1}'.format(fil_file, new_fil_name)
subprocess.call(cmd, shell=True)

fil_file = new_fil_name
...
for cand in root.findall('candidates/candidate'):
    ...
    if snr >= 10:
        ...
        if os.path.exists(mask_file_name):
            cmd = 'prepfold -noxwin -ncpus 1 -dm {0} -nsub {1} -npart {2} {3} -n {4} -npfact {5} -ndmfact {6} -p {7} -mask ../{8} ../../{9}'.format(dm, 88, npart, otheropts, N, Mp, Mdm, period, mask_file_name , fil_file)
            print period, dm, snr
            print (cmd)
#            subprocess.call(cmd, shell=True)
            job_prepfold.append(cmd)
    
        else:
            cmd = 'prepfold -noxwin -ncpus 1 -dm {0} -nsub {1} -npart {2} {3} -n {4} -npfact {5} -ndmfact {6} -p {7} ../../{8}'.format(dm, 88, npart, otheropts, N, Mp, Mdm, period, fil_file)
            print period, dm, snr
            print (cmd)
#            subprocess.call(cmd, shell=True)
            job_prepfold.append(cmd)
...
pool = Pool(ncpus)
pool.map(excute_cmd, job_prepfold)
pool.terminate()
#pool.close()
#pool.join()
