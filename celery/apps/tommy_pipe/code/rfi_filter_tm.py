...
def rfi_filter(fil_file, time, timesig, freqsig, chanfrac, intfrac, max_percent, mask, sp, outdir='/datax2/users/tp'):
    ...
    #file_name is filterbank file without .fil extension
    hdrbase = os.path.basename(file_name)
    out_path = os.path.join(outdir, hdrbase)
    if not os.path.exists(out_path):
        os.mkdir(out_path)
    os.chdir(out_path)

    hdr_file_name = os.path.basename(file_name) + ".header"
    ...
    ##! base_name = os.path.join(out_path, source_name + "_" + MJD) #create a base filename for files that will be created from the pipeline.
    ...
    if not mask:
        print("CMD: rfifind -time {0} -timesig {1} -freqsig {2} -chanfrac {3} -intfrac {4} -o {5} {6}".format(time, timesig, freqsig, chanfrac, intfrac, base_name, fil_file))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', dest='outdir', default='/datax2/users/tp', help='output directory root')
    '''     
    if(len(sys.argv) < 2):
    ...
    rfi_filter(fil_file, time, timesig, freqsig, chanfrac, intfrac, max_percent, mask, sp, outdir)
