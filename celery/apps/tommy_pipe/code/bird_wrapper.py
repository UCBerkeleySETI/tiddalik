...
def prepdata(dm_name, mask_file, fil_file): #prepdata command in PRESTO
	dm = dm_name[-4:-1]
	runsys("prepdata -nobary -o {0} -dm {1} -mask {2} -numout 530000 {3}".format(dm_name, dm, mask_file, fil_file))
	runsys("prepdata -nobary -o {0} -dm {1} -mask {2} -numout 530000 {3}".format(dm_name, dm, mask_file, fil_file))

def real_fft(dat_file): #real_fft command in PRESTO
	runsys("realfft {0}".format(dat_file))

def accel_search(dat_file, num_harm = '4', zmax = '0'): #accel_search command in PRESTO
	runsys("accelsearch -numharm {0} -zmax {1} {2}".format(num_harm, zmax, dat_file))
